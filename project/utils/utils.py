...
async def query_primary_column(email: str, wks_records: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """
    ...
    matching_users = [row for row in wks_records if row.get("Primary Email", "").lower() == email_lower]
...
def extract_from_form(form):
    ...
    secondary_email = form.secondary_email.data if hasattr(form, 'secondary_email') else ""
...
def find_user_by_email(email: str, wks_records: List[Dict[str, Any]], search_both_columns: bool = True) -> Dict[str, Any]:
    """
