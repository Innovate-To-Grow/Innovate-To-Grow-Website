...
import {buildProjectsWorksheet} from '@/features/projects/components/export/excelExport';
import {exportProjectRowsPdf} from '@/features/projects/components/export/pdfExport';
import {createProjectRowsWordBlob} from '@/features/projects/components/export/wordExport';
import {loadI2gLogoAsset} from '@/features/projects/components/export/logoAsset';
import type {ProjectGridRow} from '@/features/projects/components/projectGrid';
import {triggerDownload} from '@/features/projects/components/export/exportTypes';
...
describe('triggerDownload', () => {
    it('clicks a temporary download link and always revokes its object URL', () => {
        const blob = new Blob(['download body'], {type: 'text/plain'});
        const createObjectURL = vi.fn().mockReturnValue('blob:download-123');
        const revokeObjectURL = vi.fn();
        const click = vi.spyOn(HTMLAnchorElement.prototype, 'click').mockImplementation(() => undefined);
        vi.stubGlobal('URL', {...URL, createObjectURL, revokeObjectURL});

        triggerDownload(blob, 'past-projects.txt');

        expect(createObjectURL).toHaveBeenCalledWith(blob);
        expect(click).toHaveBeenCalledTimes(1);
        const link = click.mock.instances[0] as HTMLAnchorElement;
        expect(link.href).toBe('blob:download-123');
        expect(link.download).toBe('past-projects.txt');
        expect(link.isConnected).toBe(false);
        expect(revokeObjectURL).toHaveBeenCalledWith('blob:download-123');
