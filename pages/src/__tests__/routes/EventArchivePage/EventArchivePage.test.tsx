...
import {EventArchivePage} from '@/routes/EventArchivePage/EventArchivePage';

const mockUsePastProjectsData = vi.fn();

vi.mock('@/features/projects/hooks/usePastProjectsData', () => ({
    usePastProjectsData: (query: unknown) => mockUsePastProjectsData(query),
}));
...
describe('EventArchivePage', () => {
    ...
    it('filters archive rows to the selected event semester', () => {
        ...
        expect(mockUsePastProjectsData).toHaveBeenCalledWith({year: 2025, season: 2});
    });
});
