...
def _patch_merge_tables(monkeypatch, wks):
    fake_gc = MagicMock()
    fake_gc.open.return_value.worksheet.return_value = wks
    fake_gspread = MagicMock()
    fake_gspread.service_account.return_value = fake_gc
    monkeypatch.setattr(home, "gspread", fake_gspread)


def test_past_projects_get_without_uuid(client, monkeypatch):
    wks = MagicMock()
    _patch_merge_tables(monkeypatch, wks)

    resp = client.get("/past-projects")
    assert resp.status_code == 200
    wks.find.assert_not_called()  # no lookup without a uuid


def test_past_projects_get_with_uuid(client, monkeypatch):
    wks = MagicMock()
    cell = MagicMock()
    cell.row = 5
    wks.find.return_value = cell
    wks.row_values.return_value = ["the-uuid", "TeamA ; TeamB", "1 ; 2"]
    _patch_merge_tables(monkeypatch, wks)

    resp = client.get("/past-projects/the-uuid")
    assert resp.status_code == 200
    wks.find.assert_called_once()


def test_past_projects_post_returns_uuid(client, monkeypatch):
    wks = MagicMock()
    _patch_merge_tables(monkeypatch, wks)

    resp = client.post("/past-projects", json=[{"Team Name": "A", "Team#": "1"}])
    assert resp.status_code == 200
    body = resp.get_json()
    assert "uuid_string" in body and body["uuid_string"]
