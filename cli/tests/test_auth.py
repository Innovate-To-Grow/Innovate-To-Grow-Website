...
def test_ensure_token_logs_in_when_missing(monkeypatch):
    def fake_login(base_url, profile=None):
        config.save_credentials(
            {"base_url": base_url, "access_token": "NEW", "expires_at": time.time() + 10000}, profile=profile
        )
...
def test_ensure_token_relogins_when_expired(monkeypatch):
    ...
    def fake_login(base_url, profile=None):
        ...
        config.save_credentials(
            {"base_url": base_url, "access_token": "FRESH", "expires_at": time.time() + 10000}, profile=profile
        )
...
def test_ensure_token_raises_if_login_produces_no_token(monkeypatch):
    monkeypatch.setattr(auth, "login", lambda base_url, profile=None: None)
    with pytest.raises(AuthError):
        auth.ensure_token()
