...
from pages.models import NewsArticle, NewsFeedSource, NewsSyncLog
...
class NewsArticleAdminTest(TestCase):
    ...
    def test_changelist_accessible(self):
        resp = self.client.get(reverse("admin:pages_newsarticle_changelist"))
...
class NewsFeedSourceAdminTest(TestCase):
    ...
    def test_changelist_accessible(self):
        resp = self.client.get(reverse("admin:pages_newsfeedsource_changelist"))
        self.assertEqual(resp.status_code, 200)

    @patch("pages.admin.news.feed_source.sync_news")
    def test_sync_all_feeds(self, mock_sync):
        mock_sync.return_value = {"created": 5, "updated": 2, "errors": []}
        resp = self.client.get(reverse("admin:pages_newsfeedsource_sync_all_feeds"))
    ...
    def test_sync_this_feed(self, mock_sync):
        ...
        resp = self.client.get(reverse("admin:pages_newsfeedsource_sync_this_feed", args=[self.source.pk]))
        self.assertEqual(resp.status_code, 302)
        mock_sync.assert_called_once_with(feed_url=self.source.feed_url, source_key="ucmerced")
    ...
    def test_sync_creates_log(self, mock_sync):
        ...
        self.client.get(reverse("admin:pages_newsfeedsource_sync_all_feeds"))
...
class NewsSyncLogAdminTest(TestCase):
    ...
    def test_changelist_accessible(self):
        resp = self.client.get(reverse("admin:pages_newssynclog_changelist"))
        self.assertEqual(resp.status_code, 200)
