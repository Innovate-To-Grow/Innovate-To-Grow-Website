...
class UniformForm(TimeStampedModel, SoftDeleteModel, AuthoredModel):
    ...
    form_uuid = models.UUIDField(
        default=uuid.uuid4, editable=False, unique=True, help_text="Unique identifier for this form"
    )
    name = models.CharField(max_length=200, help_text="Human-readable form name (e.g., 'Event Registration')")
    slug = models.SlugField(max_length=100, unique=True, help_text="URL-friendly identifier for this form")
    description = models.TextField(blank=True, help_text="Optional description of the form's purpose")
    ...
    submit_button_text = models.CharField(
        max_length=100, default="Submit", help_text="Text displayed on the submit button"
    )
    success_message = models.TextField(
        default="Thank you for your submission!", help_text="Message displayed after successful submission"
    )
    redirect_url = models.URLField(blank=True, null=True, help_text="Optional URL to redirect to after submission")

    # Email Notifications
    enable_email_notification = models.BooleanField(
        default=True, help_text="Send email notification when form is submitted"
    )
    notification_recipients = models.TextField(
        blank=True, help_text="Comma-separated email addresses to notify on submission"
    )
    email_subject_template = models.CharField(
        max_length=255, blank=True, help_text="Template for email subject (leave blank for default)"
    )

    # Publishing & Status
    is_active = models.BooleanField(default=True, help_text="Whether this form accepts new submissions")
    published = models.BooleanField(default=False, help_text="Whether this form is publicly visible")

    # Submission Settings
    allow_anonymous = models.BooleanField(default=True, help_text="Allow submissions from users who are not logged in")
    login_required = models.BooleanField(default=False, help_text="Require users to be logged in to submit this form")
    max_submissions_per_user = models.PositiveIntegerField(
        default=0, help_text="Maximum submissions per user (0 = unlimited)"
    )

    # Analytics
    submission_count = models.PositiveIntegerField(default=0, help_text="Total number of submissions received")
    ...
    def validate_submission_data(self, data):
        ...
        for field_def in self.fields:
            ...
            if required and (field_name not in data or not data[field_name]):
                error_msg = validation.get("error_messages", {}).get("required", f"{field_label} is required")
