...
from ..models import UniformForm, FormSubmission
...
class UniformFormAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ('Basic Information', {
            'fields': ('name', 'slug', 'description', 'form_uuid')
        }),
        ...
        ('Submission Rules', {
            'fields': ('allow_anonymous', 'login_required', 'max_submissions_per_user')
        }),
        ('Publishing', {
            'fields': ('is_active', 'published')
        }),
        ('Statistics', {
            'fields': ('submission_count', 'created_at', 'updated_at'),
            'classes': ('collapse',)
        }),
    )
...
class FormSubmissionAdmin(admin.ModelAdmin):
    ...
    readonly_fields = (
        'submission_uuid', 'form', 'user', 'data', 'submitted_at',
        'ip_address', 'user_agent', 'referrer', 'notification_sent_at'
    )
    date_hierarchy = 'submitted_at'

    fieldsets = (
        ('Submission Information', {
            'fields': ('submission_uuid', 'form', 'user', 'submitted_at')
        }),
        ('Submitted Data', {
            'fields': ('data',),
            'classes': ('extrapretty',)
        ...
        ('Email Notification', {
            'fields': ('notification_sent', 'notification_sent_at', 'notification_error'),
            'classes': ('collapse',)
        }),
    )
    ...
    def mark_as_processed(self, request, queryset):
        ...
        count = queryset.update(
            status=FormSubmission.STATUS_PROCESSED,
            updated_at=timezone.now()
        )
