...
from .rewrite import _RUN_BODY_PATHS, _rewrite_json_body_user_id, _rewrite_scope_user_id, _scope_adk_path
...
class AdminADKWebAuthMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        adk_path = _scope_adk_path(scope)
        user_pk = await self.user_loader(scope.get("headers", []))
        if user_pk is None:
            if scope["type"] == "websocket":
                await send({"type": "websocket.close", "code": 1008})
            elif _is_browser_shell_path(adk_path):
        ...
        if rewritten_scope["type"] == "http" and _scope_adk_path(rewritten_scope) in _RUN_BODY_PATHS:
