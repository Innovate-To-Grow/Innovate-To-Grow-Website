...
from cms.services.news.feed_parser import extract_image_url, extract_summary, parse_pub_date
from cms.services.news.scraper import scrape_article
...
class ScraperTest(TestCase):
    @patch("cms.services.news.scraper.urllib.request.urlopen")
...
class SyncNewsTest(TestCase):
    @patch("cms.services.news.sync.scrape_article", side_effect=Exception("scrape error"))
    @patch("cms.services.news.sync.fetch_feed")
    ...
    @patch("cms.services.news.sync.scrape_article")
    @patch("cms.services.news.sync.fetch_feed")
    ...
    @patch("cms.services.news.sync.scrape_article", side_effect=Exception("scrape error"))
    @patch("cms.services.news.sync.fetch_feed", side_effect=Exception("Network error"))
