"""
Transaction model admin configuration with barcode scanning capability.
"""
...
class TransactionAdmin(ModelAdmin):
    ...
    def scan_lookup_view(self, request):
        ...
        barcode = Barcode.objects.filter(barcode=barcode_value, is_deleted=False).select_related("model_user").first()
        if not barcode:
            return JsonResponse(
                {
                    "found": False,
