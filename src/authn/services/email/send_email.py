...
def send_admin_invitation_email(*, invitation, request=None):
    ...
    Unlike general notifications, this raises if delivery fails so the Django
    admin can tell staff that the invitation record was created but not sent.
    """
    config = _load_config()
    ...
    elif _send_via_ses(config=config, recipient=invitation.email, subject=subject, html_body=html_body):
        logger.info("Admin invitation email sent via SES")
        return
    else:
        logger.warning("SES send failed; falling back to SMTP for admin invitation (host=%s)", config.smtp_host)

    _send_via_smtp(config=config, recipient=invitation.email, subject=subject, html_body=html_body)
