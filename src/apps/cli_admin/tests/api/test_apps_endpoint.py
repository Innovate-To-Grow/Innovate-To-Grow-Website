...
class AppListEndpointTests(CliApiTestCase):
    ...
    def test_app_list_does_not_enumerate_ungranted_apps(self):
        staff = make_staff(email="apps-scoped@example.com", apps=["projects"])
        _, raw = issue_token(staff)

        response = self.client.get("/admin-api/apps/", **self.auth(raw))

        self.assertEqual(response.status_code, 200)
        self.assertEqual({row["app_label"] for row in response.data}, {"projects"})
