(function () {
    ...
    function renderFooterHtml(content) {
        ...
        const cta = (content.cta_buttons || []).map(btn => `<div class="sb-col hb__buttons-${btn.style === 'gold' ? 'gold' : 'blue'}"><a class="btn--invert-${btn.style === 'gold' ? 'gold' : 'blue'} hb__play" href="${escapeAttr(btn.href)}" data-preview-link>${escapeHtml(btn.label)}</a></div>`).join('');
        ...
        const columns = (content.columns || []).map((col, idx, arr) => `<div class="footer-column${idx === arr.length - 1 ? ' footer-column--address' : ''}">${col.title ? `<h2>${escapeHtml(col.title)}</h2>` : ''}${(col.links || []).length ? `<ul>${(col.links || []).map(link => `<li><a href="${escapeAttr(link.href)}" target="${escapeAttr(link.target || '_blank')}" rel="${escapeAttr(link.rel || 'noopener')}" data-preview-link>${escapeHtml(link.label)}</a></li>`).join('')}</ul>` : ''}${col.body_html ? `<div class="footer-column__body">${col.body_html}</div>` : ''}${idx === arr.length - 1 && (content.social_links || []).length ? `<div class="socialIcons"><ul class="fa-ul inline">${(content.social_links || []).map(s => `<li class="fa-li"><a href="${escapeAttr(s.href)}" target="${escapeAttr(s.target || '_blank')}" rel="${escapeAttr(s.rel || 'noopener')}" aria-label="${escapeAttr(s.aria_label)}" data-preview-link><i class="${escapeAttr(s.icon_class)}"></i></a></li>`).join('')}</ul></div>` : ''}</div>`).join('');
        const footerLinks = (content.footer_links || []).map(link => `<li><a href="${escapeAttr(link.href)}" target="${escapeAttr(link.target || '_blank')}" rel="${escapeAttr(link.rel || 'noopener')}" data-preview-link>${escapeHtml(link.label)}</a></li>`).join('');
    ...
    function updatePreview(iframe, errorBox, footerData) {
        ...
        try {
            ...
            iframe.srcdoc = `<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><link rel="stylesheet" href="${FONT_AWESOME_CSS}"><style nonce="${nonce}">${inlineCSS}</style></head><body>${renderFooterHtml(footerData)}</body></html>`;
